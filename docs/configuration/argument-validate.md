---
title: "Argument: Validate"
sidebarDepth: 2
---

# Using the `validate` Argument

There are many validation options included within Redux that can apply to [fields](object-field.md). You can 
...
## How to Use
You declare the use of a validation by passing the `validate` argument to the [field array](object-field.md).
Below is an example:
...
|`date`|Date|
|`email`}|Valid Email|
|`html_custom`|HTML validation - See [html_custom example](./argument-validate.md#html-custom-example) |
|`js`|JavaScript|
...
|`preg_replace`|Peform a replace on the contents based on a regex pattern. See [preg_replace Example](./argument-validate.md#preg-replace-example)|
|`str_replace`|Return a string after running through a `str_replace`. See [str_replace Example](./argument-validate.md#str-replace-example)|
...
## A Warning
::: danger
The following fields do **NOT** accept validation unless using [validate_callback](./argument-validate.md#using-a-custom-validation).
